...
while running:
    ...
    # Calculate Drone 2 delivery time for all found humans
    time2 = calculate_drone2_delivery_time(drone1_found)
    
    # Launch condition: rest_time <= time2 AND we have at least one person to deliver to
    ...
    # ----------------- Drone 2: Delivery Operations -----------------
    if drone2_mode == "delivering":
        if drone2_delivery_timer > 0:
            ...
            # Finished current path - check for more targets or return
            ...
            if remaining_undelivered and len(drone2_delivered) < 10:
                ...
                # All found targets delivered or mission complete
    ...
    # Time analysis
    d1_time_left = max(0, int(rest_time / FPS))
    d2_time_needed = max(0, int(time2 / FPS))
    WIN.blit(SMALL_FONT.render(f"D1 time left: {d1_time_left}s", True, BLACK), (status_x, status_y + 100))
    WIN.blit(SMALL_FONT.render(f"D2 time needed: {d2_time_needed}s", True, BLACK), (status_x, status_y + 115))
    
    # Launch condition indicator
    launch_ready = rest_time <= time2 and drone1_found
    color = GREEN if launch_ready else RED
    status_text = "READY" if launch_ready else "WAITING"
    WIN.blit(SMALL_FONT.render(f"Launch status: {status_text}", True, color), (status_x, status_y + 130))
