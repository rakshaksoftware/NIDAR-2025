#!/usr/bin/env python3
"""
delivery_combined_mission.py

- Listens for HUMAN STATUSTEXT (HUMAN,lat=...,lon=...,alt=...,conf=... or H,lat,lon,alt,conf)
- Logs detections to detections_log.csv
- Runs DBSCAN to deduplicate and write waypoints.csv
- When >= MIN_FOR_START clusters found, spawns the delivery mission that visits the NEXT 5 waypoints and triggers a servo
- After RTL + landing + disarm, it waits for pilot to set GUIDED again; then the listener will trigger the next mission
  using the new detections collected while flying (pending_detections rolled into active_detections).
"""
...
def parse_statustext_text(text: str):
    """
    Parse STATUSTEXT text into dict with keys: lat, lon, alt, conf
    Accepts:
      - "HUMAN,lat=...,lon=...,alt=...,conf=..."
      - "H,19.1337,72.9158,12.3,0.92"
    Returns dict or None if parse failed.
    """
    s = text.strip()
    if s.startswith("HUMAN"):
        ...
        if "lat" in out and "lon" in out:
            c = out.get("conf", None)
            if c is None:
                c = out.get("co", None)  # fallback if your sender uses "co"
            return {
                "lat": float(out["lat"]),
                "lon": float(out["lon"]),
...
def mission_runner(fc_conn, waypoints):
    ...
    try:
        ...
        # go through ONLY the next 5 waypoints (one payload each)
        waypoints = waypoints[:len(SERVO_NUMBERS)]
        ...
        for i, wp in enumerate(waypoints, start=1):
            ...
            ok, dist, alt = goto_waypoint_guided(
                ...
                radius_m=ARRIVAL_RADIUS_M, timeout=WP_TIMEOUT
            )
            if not ok:
            ...
            print(f"[MISSION] Trigger servo {servo_num} PWM={SERVO_PWM_TRIGGER}")
        ...
        print("[MISSION] Landed. Disarming...")
        safe_disarm(master)
        print("[MISSION] Done")
        ...
        # IMPORTANT: roll pending -> active so next run uses NEW detections gathered while flying
...
def listener_thread(listen_conn, min_for_start):
    ...
    while True:
        ...
        # Debug: print all STATUSTEXT messages received
        ...
        # store in active or pending batch
        with detections_lock:
            if mission_running:
        ...
        # run dedupe on ACTIVE only
        try:
            n_clusters, waypoints = run_dbscan_and_write_waypoints(
                df_list, cluster_radius_m=CLUSTER_RADIUS_METERS, out_csv=WAYPOINTS_CSV
            )
        except Exception as e:
        ...
        if n_clusters >= min_for_start:
            # Manual pilot approval gate: must be in GUIDED
